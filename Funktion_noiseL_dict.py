...
"""
Function for simulating the system with continuous noise applied to L, using sdeint as a solver.
Parameters are the initial condition, the simulation time, the step size as well as the amplitude of the applied noise.
There is one file for each variable noise is applied to.
"""

np.set_printoptions(threshold=sys.maxsize)

def run_noise(init, Time, step, amp):
    """
    ...
    def differential_values(logx, unused_t):
        ...
        This function will be used in sdeint.
        """
    ...
    N = np.diag([0., 1., 0., 0.]) # works on log(L), always put abs(N) = 1 and choose a low amplitude at defining the noise matrix

    #defining the noise matrix, amp is given at the function call
    def noise(logx, unused_t):
        return amp*N


    #running the whole system with sdeint, with flow-function, inital value array, time array and noise-array
    logx = sde.itoSRI2(differential_values, noise, logx0, t)
