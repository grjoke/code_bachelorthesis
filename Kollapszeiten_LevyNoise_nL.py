...
disturbed by discrete noise applied on one variable (L).
It can be called with "python3 Kollapszeiten_LevyNoise_nL.py" .
...
if os.path.isdir('files_collaps_rate_LevyNoise_nL'):
    shutil.rmtree('files_collaps_rate_LevyNoise_nL')
os.mkdir('files_collaps_rate_LevyNoise_nL')
#function to save arrays in a textfile in the directory which was created before
def write_results_in_file(array, fname):
    if os.path.exists('files_collaps_rate_LevyNoise_nL/' + fname + '.txt'):
        os.remove('files_collaps_rate_LevyNoise_nL/' + fname + '.txt')
    print(array, file = open('files_collaps_rate_LevyNoise_nL/' + fname + '.txt', 'a'))
    return 0
...
def run_model_with_different_amplitudes(x_init, noise_end):
    ...
    for i in range(n):
        ...
        sim,_ = fcln.run_LeviNoise_L(x_init, T, dt, pm.amplitude_fc)
        L = sim['L']/pm.Cges
        for k in range(T):
            if L[k] < 0.01:
                collaps_rate[i] = 1/k
...
def plot_the_results(amplitude_array, collaps_rate_array, amplitude_plot_array, res_regression, fname):
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig('files_collaps_rate_LevyNoise_nL/' + fname + '.pdf')
    plt.savefig('files_collaps_rate_LevyNoise_nL/' + fname + '.png')
    #plt.show()
    plt.clf()

#running the plot function
plot_the_results(amplitudes_1, collaps_rate_1, amplitude_plot_array, res1, fname = 'collaps_rate_LevyNoise_1_nL')
plot_the_results(amplitudes_2, collaps_rate_2, amplitude_plot_array, res2, fname = 'collaps_rate_LevyNoise_2_nL')
