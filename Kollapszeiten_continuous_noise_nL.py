...
disturbed by continuous noise applied on one variable (L).
It can be called with "python3 Kollapszeiten_continuous_noise_nL.py" .
...
if os.path.isdir('files_collaps_rate_gaussian_nL'):
    shutil.rmtree('files_collaps_rate_gaussian_nL')
os.mkdir('files_collaps_rate_gaussian_nL')
#function to save arrays in a textfile in the directory which was created before
def write_results_in_file(array, fname):
    if os.path.exists('files_collaps_rate_gaussian_nL/' + fname + '.txt'):
        os.remove('files_collaps_rate_gaussian_nL/' + fname + '.txt')
    print(array, file = open('files_collaps_rate_gaussian_nL/' + fname + '.txt', 'a'))
    return 0
...
def plot_the_results(amplitude_array, collaps_rate_array, amplitude_plot_array, res_regression, fname):
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig('files_collaps_rate_gaussian_nL/' + fname + '.pdf')
    plt.savefig('files_collaps_rate_gaussian_nL/' + fname + '.png')
    #plt.show()
    plt.clf()

#running the plot function
plot_the_results(amplitudes_1, collaps_rate_1, amplitude_plot_array, res1, fname = 'collaps_rate_gaussian_noise_1_nL')
plot_the_results(amplitudes_2, collaps_rate_2, amplitude_plot_array, res2, fname = 'collaps_rate_gaussian_noise_2_nL')
