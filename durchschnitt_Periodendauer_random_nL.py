"""
Python file for conducting a statistic experiment about the average period time of the system with L-noise applied.
It can be called with "python3 durchschnittliche_Periodendauer_random_nL.py" .
...
if os.path.isdir('files_aver_period_time_nL'):
    shutil.rmtree('files_aver_period_time_nL')
os.mkdir('files_aver_period_time_nL')

#function to write the results of a step in a textfile
def write_results_in_file(array, fname):
    if os.path.exists('files_aver_period_time_nL/' + fname + '.txt'):
        os.remove('files_aver_period_time_nL/' + fname + '.txt')
    print(array, file = open('files_aver_period_time_nL/' + fname + '.txt', 'a'))
    return 0
...
def find_maxima_in_oscillation(value_array):
    ...
    timepeaks,_ = find_peaks(value_array_smooth, height = 1.28e9, distance = 600)
    return timepeaks
...
if os.path.exists('files_aver_period_time_nL/param_cyclefit_period_time_random_1.txt'):
    os.remove('files_aver_period_time_nL/param_cyclefit_period_time_random_1.txt')
print('amp = ' + str(popt_1[0]) + ', k = ' + str(popt_1[1]) + ', cx = ' + str(popt_1[2]) + ', cy = ' + str(popt_1[3]) + ', error = ' + str(err1), file = open('files_aver_period_time_nL/param_cyclefit_period_time_random_1.txt', 'a'))
if os.path.exists('files_aver_period_time_nL/param_cyclefit_period_time_random_2.txt'):
    os.remove('files_aver_period_time_nL/param_cyclefit_period_time_random_2.txt')
print('amp = ' + str(popt_2[0]) + ', k = ' + str(popt_2[1]) + ', cx = ' + str(popt_2[2]) + ', cy = ' + str(popt_2[3]) + ', error = ' + str(err2), file = open('files_aver_period_time_nL/param_cyclefit_period_time_random_2.txt', 'a'))
...
def plot_the_results(amplitude_array, period_time_array, amplitude_cycles, cycle_array, res_regression, popt, fname):  #popt,
    ...
    plt.title('Average period time dependent on the noise amplitude \n (continuous noise on $L$)')
    plt.ylim(500, 3500)
    plt.legend()
    plt.grid()
    plt.tight_layout()
    plt.savefig('files_aver_period_time_nL/' + fname + '.pdf')
    plt.savefig('files_aver_period_time_nL/' + fname + '.png')
    #plt.show()
    plt.clf()

    sns.regplot(x = amplitude_array, y = period_time_array, robust = False, scatter_kws={'alpha':0.5}, label = 'average period time',  marker = 'o', color = 'g')
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig('files_aver_period_time_nL/' + fname + '_seaborn.pdf')
    plt.savefig('files_aver_period_time_nL/' + fname + '_seaborn.png')
    #plt.show()
    plt.clf()
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig('files_aver_period_time_nL/' + fname + '_cycles.pdf')
    plt.savefig('files_aver_period_time_nL/' + fname + '_cycles.png')
    #plt.show()
    plt.clf()
...
plot_the_results(amplitudes_1, average_period_time_1, amplitudes_cycles_1, number_of_cycles_1, res1, popt_1, fname = 'average_period_time_gaussian_noise_1_nL') #popt_1,
plot_the_results(amplitudes_2, average_period_time_2, amplitudes_cycles_2, number_of_cycles_2, res2, popt_2, fname = 'average_period_time_gaussian_noise_2_nL') #popt_2,
